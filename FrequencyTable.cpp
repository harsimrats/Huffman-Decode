...
CodeTree FrequencyTable::buildCodeTree() const {
	...
	// Add leaves for symbols with non-zero frequency
	{
		...
		for (uint32_t freq : frequencies) {
			if (freq > 0)
			{
				...
				temp->symbol = i;
				pqueue.push(NodeWithFrequency(temp, i, freq));
			}
	...
	while (pqueue.size() > 1) {
		...
		iNode* temp = (iNode*) malloc(sizeof(iNode));
		temp->leftChild = x.node;
		temp->rightChild = y.node;
...
FrequencyTable::NodeWithFrequency::NodeWithFrequency(iNode *nd, uint32_t lowSym, uint64_t freq) :
	node(nd),
	lowestSymbol(lowSym),
	frequency(freq) {}
